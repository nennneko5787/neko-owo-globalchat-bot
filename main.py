...
def run_supabase_query(func, *args):
	loop = asyncio.get_event_loop()
	return loop.run_in_executor(None, func, *args)


async def run_supabase_query_async(func, *args):
	return await run_supabase_query(func, *args)
...
async def on_guild_join(guild):
	...
	embed = discord.Embed(
		title="neko's global chat Botを導入していただきありがとうございます。",
		description="早速このチャンネルで挨拶をしてみましょう！",
		color=0xDA70D6,
	)
...
async def user(interaction: Interaction, message: Message):
	...
	if message.channel.name == "neko-global-chat":
		# カーソルをオープンします
		response = await run_supabase_query_async(
			supabase.table("message").select("*").eq("message", message.id).execute
		)
		...
		embed.add_field(
			name="このサーバーに参加した日時",
			value=user.joined_at.astimezone(
				datetime.timezone(datetime.timedelta(hours=9))
			).strftime("%Y/%m/%d %H:%M:%S.%f"),
		)
		embed.add_field(
			name="Discordに登録した日時",
			value=user.created_at.astimezone(
				datetime.timezone(datetime.timedelta(hours=9))
			).strftime("%Y/%m/%d %H:%M:%S.%f"),
		)
...
async def user(interaction: Interaction, message: Message):
	...
	if message.channel.name == "neko-global-chat":
		...
		embed.add_field(
			name="サーバーの作成日時",
			value=guild.created_at.astimezone(
				datetime.timezone(datetime.timedelta(hours=9))
			).strftime("%Y/%m/%d %H:%M:%S.%f"),
		)
		await interaction.followup.send("", embed=embed, ephemeral=True)
	...
	if interaction.user.id == 1048448686914551879:
		if message.channel.name == "neko-global-chat":
			...
			response = await run_supabase_query_async(
				...
				.select("*")
				.eq("raw_message", query_result["raw_message"])
				.execute
			)
			query_result = response.data[0]
...
async def on_message_delete(message):
	if message.channel.name == "neko-global-chat":
		...
		query_result = response.data[0]
		if query_result["raw_message"] == message.id:
			response = await run_supabase_query_async(
				supabase.table("message")
				.select("*")
				.eq("raw_message", query_result["raw_message"])
...
async def test_command(interaction: discord.Interaction):
	...
	for guild in client.guilds:
		true_member_count = len([m for m in guild.members if not m.bot])
		embed.add_field(
			name=f"{guild.name}({true_member_count} / {guild.member_count})",
...
@tree.command(
	name="generate",
	description="Loop all guilds, If #neko-global-chat channel is not found, then create #neko-global-chat channel.",
)
async def test_command(interaction: discord.Interaction):
	if interaction.user.id != 1048448686914551879:
		await interaction.response.send_message(
		...
		if cnl == None:
			...
			try:
				...
				await interaction.channel.send(
					f"Error(おそらく権限が足りない): {guild.name}({guild.id})"
				)
				if guild.id != 861590137116819486:
					await guild.owner.create_dm()
					embed = discord.Embed(
						title="neko's global chat botについて",
						description="こんにちは。あなたのサーバーで、neko's global chat botが正しく導入されていないことが確認されました。\nですので、Discordサーバーにて`neko-global-chat`という名前のテキストチャンネルを作成していただければなと思います。\nこの件についてのお問い合わせは、このDMではなく、**nennneko5787**のDMまでどうぞ。",
						color=0xDA70D6,
					)
					embed.add_field(name="正しく導入されていないことが確認できたサーバー", value=guild.name)
					embed.set_author(
						name="nennneko5787", icon_url="https://i.imgur.com/zJ094I4.png"
					)
					await guild.owner.dm_channel.send("", embed=embed)
					await interaction.channel.send(f"DMおくった: {guild.name}({guild.id})")
				else:
					await interaction.channel.send(
						f"ディス速のBot置き場でした: {guild.name}({guild.id})"
					)
		else:
...
async def on_message(message):
	...
	if message.channel.name == global_channel_name:  # グローバルチャットにメッセージが来たとき
		...
		if (
			message.content.find("discord.gg") != -1
			or message.content.find("discord.com/invite/") != -1
			or message.content.find("dsc.gg") != -1
		):
			await message.author.create_dm()
			embed = discord.Embed(
				title="エラーが発生しました。",
				description="禁止ワードが含まれています",
		...
		if (time.time() - spamtaisaku[f"{message.author.id}"]) <= 8:
			...
			embed = discord.Embed(
				title="エラーが発生しました。",
				description=f"スパムは禁止です。あと{8 - otintin}秒待ってからもう一度メッセージを送信してさい。",
				color=discord.Colour.red(),
			)
			await message.author.dm_channel.send("", embed=embed)
			return
			...
			try:
				if channel.name == global_channel_name:  # グローバルチャット用のチャンネルが見つかったとき
					if channel != message.channel:  # 発言したチャンネルには送らない
						...
						jst_datetime = message.created_at.astimezone(
							datetime.timezone(datetime.timedelta(hours=9))
						)
						embed.set_footer(
							text="{} | {}".format(
						...
						if message.reference:  # 返信メッセージであるとき
							...
							if (
								...
								and reference_msg.author == client.user
...
async def on_reaction_add(reaction, user):
	if user.id != reaction.message.guild.me.id:
		response = await run_supabase_query_async(
			supabase.table("message")
			.select("*")
			.eq("message", reaction.message.id)
			.execute
		)
		query_result = response.data[0]
	...
	await client.change_presence(
		activity=discord.Game(
			name="#neko-global-chat | {} Servers | deployed: {}".format(
				len(client.guilds), last_commit_date
			)
		)
	)
